                            ...
                            at::Tensor &lora_ranks, at::Tensor &lora_scales, at::Tensor &y)
{
    ...
    int32_t dataType = ConvertDataType(scalar_type);

    int32_t block_dim;
    int32_t workspace_size;
    int64_t total_extend_tokens = out_indices.sizes()[0];

    at::Tensor tiling_tensor = get_shrink_tiling(block_dim, workspace_size, batch_size, total_extend_tokens);

    auto workspace_tensor =
        at::empty({workspace_size}, at::TensorOptions().dtype(at::kByte).device(x.options().device()));
    /* launch the kernel function via torch */
    EXEC_KERNEL_CMD(sgemmv_shrink, block_dim, x_ptr, weight_ptr, lora_indices_ptr, lora_indices_size, seq_len_ptr,
                    seq_len_size, lora_ranks_ptr, lora_ranks_size, y_ptr, batch_size, num_tokens_per_core,
